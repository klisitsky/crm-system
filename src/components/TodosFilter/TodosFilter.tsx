...
import s from "@/components/TodosFilter/TodosFilter.module.scss";
  ...
  ({ isLoading, filterStatus, todoInfo, setfilterStatus }) => {
    ...
    return (
      <Flex justify="space-around" style={{ padding: "5px" }}>
        <Button type={"text"} disabled={isLoading} onClick={() => setfilterStatus("all")}>
          <Text className={`${filterStatus === "all" ? s.active : ""}`}>{`Все (${all})`}</Text>
        </Button>
        <Button type={"text"} disabled={isLoading} onClick={() => setfilterStatus("inWork")}>
          <Text
            className={`${filterStatus === "inWork" ? s.active : ""}`}
          >{`В работе (${inWork})`}</Text>
        </Button>
        <Button type={"text"} disabled={isLoading} onClick={() => setfilterStatus("completed")}>
          <Text
